...
export default function AddIndustry() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // All required fields based on the error message
      const payload = {
        name: name.trim(),
        slug: slug,
        status: status,
        ...
        sort_order: 1,
        is_trending: false,
        created_by: 1, // Required field - using default admin user
        updated_by: 1, // Required field - using default admin user
        icon: null, // Optional - can be null
      };
      ...
      if (!res.ok) {
        throw new Error(responseData.message || "Failed to create industry.");
      }
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm max-w-7xl w-full p-6 md:p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            />
            <p className="mt-1 text-[11px] text-gray-400">
              Active industries will be visible to users
            </p>
          </div>
