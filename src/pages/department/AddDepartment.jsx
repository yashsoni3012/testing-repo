...
export default function AddDepartment() {
  ...
  const [form, setForm] = useState({
    department_name: "",
    icon: null, // Changed from string to file object
  ...
  const validate = () => {
    ...
    if (!form.icon) errs.icon = "Icon image is required.";
    return errs;
  };
  ...
  const handleFileChange = (e) => {
    ...
    if (file) {
      ...
      if (!validTypes.includes(file.type)) {
        setErrors((prev) => ({ 
          ...prev, 
          icon: "Please upload a valid image file (JPG, PNG, GIF, WEBP)" 
        }));
        return;
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formData.append("icon", form.icon); // The file
      ...
      showToast(err.message || "Failed to create department. Please try again.", "error");
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <div className="p-6 max-w-2xl">
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <form onSubmit={handleSubmit} className="space-y-5">
            {/* Department Name - same as before */}
            <div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center justify-center gap-2">
                      <span className="text-sm text-gray-600">
                        {form.icon.name} ({(form.icon.size / 1024).toFixed(1)} KB)
                      </span>
                      <button
                        type="button"
                        onClick={removeIcon}
